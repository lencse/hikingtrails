...
async function load() {
    const init = 'https://www.kektura.hu/ak-szakaszok'
    let d: any = await httpGet(init)
    d = getLinkUrlsFromHtml(d)
    d = ((args: any): any => ({
        pathGpxUrl: findByPattern(args.links, /ak_teljes_[0-9_]+\.gpx/),
        stampGpxUrl: findByPattern(args.links, /ak_bh_[0-9_]+\.gpx/)
    }))(d)
